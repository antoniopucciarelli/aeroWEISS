...
for i=1:length(alpha_vec)
    ...
    [~,alpha_ind1]   = compute_INDUCEDvel(GAMMA(1:N(1)*2*M(1)),    PANELwing(1:N(1)*2*M(1)),    alpha,M(1),N(1),U,"no"); 
    [~,alpha_ind2]   = compute_INDUCEDvel(GAMMA(N(1)*2*M(1)+1:end),PANELwing(N(1)*2*M(1)+1:end),alpha,M(2),N(2),U,"no"); 
