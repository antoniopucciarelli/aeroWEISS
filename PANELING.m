function [PANELwing] = PANELING(delta,lambda,alpha,root,taper,L,M,N,flag,transl)
...
ext1 = ext4 + L * [ + sin(lambda), + cos(lambda)];
ext2 = ext1 + root/taper * [1,0];
...
% translation process -- all the wing 
for i=1:N*2*M
   PANELwing(i).VERTEX(1,:) = PANELwing(i).VERTEX(1,:) + transl;
   PANELwing(i).VERTEX(2,:) = PANELwing(i).VERTEX(2,:) + transl;  
   PANELwing(i).VERTEX(3,:) = PANELwing(i).VERTEX(3,:) + transl;
   PANELwing(i).VERTEX(4,:) = PANELwing(i).VERTEX(4,:) + transl;
   PANELwing(i).C4(1,:)     = PANELwing(i).C4(1,:)     + transl;  
   PANELwing(i).C4(2,:)     = PANELwing(i).C4(2,:)     + transl;
   PANELwing(i).MIDPOINT    = PANELwing(i).MIDPOINT    + transl;
end
